#!/bin/sh
# kab: Read the Word of God in Georgian from your terminal
...
while [ $# -gt 0 ]; do
	...
	if [ "$1" = "--" ]; then
		...
		get_data ka_biblia.tsv | awk -v cmd=list "$(get_data kab.awk)"
		exit
	elif [ "$1" = "-W" ]; then
...
if [ $# -eq 0 ]; then
	...
	while true; do
		...
		get_data ka_biblia.tsv | awk -v cmd=ref -v ref="$ref" "$(get_data kab.awk)" | ${PAGER}
	done
	exit 0
fi

get_data ka_biblia.tsv | awk -v cmd=ref -v ref="$*" "$(get_data kab.awk)" | ${PAGER}
