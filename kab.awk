...
cmd == "ref" && mode == "exact" && bookmatches($1, $3, p["book"]) && (p["chapter"] == "" || $5 == p["chapter"]) && (p["verse"] == "" || $6 == p["verse"]) {
	processline()
}

cmd == "ref" && mode == "exact_set" && bookmatches($1, $3, p["book"]) && (p["chapter"] == "" || $5 == p["chapter"]) && p["verse", $6] {
	processline()
}

cmd == "ref" && mode == "range" && bookmatches($1, $3, p["book"]) && ((p["chapter_end"] == "" && $5 == p["chapter"]) || ($5 >= p["chapter"] && $5 <= p["chapter_end"])) && (p["verse"] == "" || $6 >= p["verse"]) && (p["verse_end"] == "" || $6 <= p["verse_end"]) {
	processline()
}

cmd == "ref" && mode == "range_ext" && bookmatches($1, $3, p["book"]) && (($5 == p["chapter"] && $6 >= p["verse"] && p["chapter"] != p["chapter_end"]) || ($5 > p["chapter"] && $5 < p["chapter_end"]) || ($5 == p["chapter_end"] && $6 <= p["verse_end"] && p["chapter"] != p["chapter_end"]) || (p["chapter"] == p["chapter_end"] && $5 == p["chapter"] && $6 >= p["verse"] && $6 <= p["verse_end"])) {
	processline()
}

cmd == "ref" && mode == "search" && (p["book"] == "" || bookmatches($1, $3, p["book"])) && (p["chapter"] == "" || $5 == p["chapter"]) && match(tolower($7), tolower(p["search"])) {
	processline()
}
